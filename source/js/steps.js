      ...
      [
        {
          ...
          value: function stepClick(e) {
            e.preventDefault()
            var nextStep = $__default['default'](this)
              .closest('[data-step-target]')
              .index()
        ...
        },
        {
          ...
          value: function getStepIndex() {
            var stepIndex = this.el
              .find(this.stepSelector)
              .filter('.'.concat(this.options.activeClass.split(' ').join('.')))
              .index()
            return stepIndex || 0
          },
        },
        ...
        },
        {
          ...
          value: function setFooterBtns() {
            ...
            if (stepIndex === 0) {
              ...
              $footer.find('[data-step-action="next"]').css('display', 'none')
              if ($('.order__info').css('display') == 'none') {
                $footer.find('[data-step-action="next"]').hide()
              }
              if ($('.order__info').css('display') == 'block') {
                $footer
                  .find('[data-step-action="next"]')
...
function radioResult() {
  var idRadio = $("input[name='radio']:checked").attr('id')
  var result = $("label[for='" + idRadio + "']").text()
  return result
}

if ($('.order__step').is('#tab-order')) {
  $('.step-btn').click(function () {
    $('.summary')
      .append(
        '<div class="summary__info">' +
          '<h3 class="summary__title"> 1. Доставка </h3>' +
          '<a href=""> Редактировать </a>' +
          '</div>' +
          '<div class="summary__all">' +
          '<p>' +
          $('#order__adress-street').val() +
          ',' +
          $('#order__adress-house').val() +
          '-' +
          $('#order__adress-flat').val() +
          ', ' +
          $('#order__city').val() +
          ', ' +
          $('#order__zip').val() +
...
if ($('.step-tab-panel').is('#tab-contacts')) {
  $('.step-btn')
    .addClass('contacts_block')
    .click(function () {
      $('.summary_contacts')
        .append(
          '<div class="summary_contacts__info">' +
            '<h3 class="summary_contacts__title"> 2. Контакты </h3>' +
            '<a href=""> Редактировать </a>' +
            '</div>' +
            '<div class="summary_contacts__all">' +
            '<div>' +
            '<p>' +
            $('#order__name').val() +
            ' ' +
